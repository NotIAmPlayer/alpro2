    ...
    try:
        ...
        for k, v in enumerate(settings):
            if type(settings[setting_names[k]]) == float:
                ...
                settings[setting_names[k]] = bool(temp[k])
...
npc_cfg = {
    'width'                     : [8, 16, 8, 16, 8, 24, 18, 16],
    'height'                    : [8, 16, 8, 16, 8, 32, 12, 12],
    'gfxwidth'                  : [8, 18, 8, 16, 8, 24, 18, 16],
    'gfxheight'                 : [8, 18, 8, 16, 8, 32, 12, 12],
    'gfxoffsetX'                : [0, -1, 0, 0, 0, 0, 0, 0],
    'gfxoffsetY'                : [0, -2, 0, 0, 0, 0, 0, 0],
    'frames'                    : [2, 4, 1, 3, 3, 5, 2, 3],
    'framestyle'                : [1, 1, 0, 0, 0, 1, 1, 0],
    'framedelay'                : [0, 0, 0, 8, 8, 0, 0, 8],
    ...
    'hittable'                  : [False, True, False, False, False, True, False, False],
    'dropsItems'                : [False, True, False, False, False, True, False, False],
}
        ...
        if self.isValid:
            if self.isActive:
                ...
                if self.id == 2: # TRACKER
                    ...
                    if currentLevel.player.immuneFrames == 0 and (currentLevel.player.dashTimer == 0 or not currentLevel.player.hasFlashDash):
    ...
    def update(self):
        ...
        if not self.isUpdating:
            if not player.hasDied:
                ...
                self.x += self.dx
                self.y += self.dy
                self.x = math_clamp(self.x, currentLevel.sections[player.section].x, currentLevel.sections[player.section].x + currentLevel.sections[player.section].width - self.width)
                self.y = math_clamp(self.y, currentLevel.sections[player.section].y, currentLevel.sections[player.section].y + currentLevel.sections[player.section].height - self.height)
                self.dx = 0
                self.dy = 0
    ...
    while True:
        ...
        if gameVars["inGame"]:
            ...
            if not gameVars["isPaused"]:
                ...
                if currentLevel.player.weapon > 0:
                    screen.blit(bars[7], (24, 78 - 36 * 2))

                    for i in range(1, currentLevel.player.weaponEnergies[currentLevel.player.weapon] + 1):
                        y = 78 - 2 * i
                        screen.blit(bars[currentLevel.player.weapon], (24, y))'''

                ## NEW HUD
                screen.blit(bars[11], (0, 0))
                screen.blit(bars[8], (16, 8), (36 - 6 * gameData["energyUpObtained"], 0, currentLevel.player.maxHealth * 2 + 1, 8))
                for i in range(1, currentLevel.player.health + 1):
                    x = 16 + 2 * (i - 1)
                    screen.blit(bars[0], (x, 8))
                screen.blit(bars[7], (16, 8), (36 - 6 * gameData["energyUpObtained"], 0, currentLevel.player.maxHealth * 2 + 1, 8))
                ...
                if currentLevel.player.weapon != 0:
                    ...
                    for i in range(1, currentLevel.player.weaponEnergies[currentLevel.player.weapon] + 1):
                        x = 16 + 2 * (i - 1)
                        screen.blit(bars[currentLevel.player.weapon], (x, 15))
                ...
                if keypressed and gameVars["menuTransitionTimer"] == 0:
                    if keypressed[controls['pause']] and not gameVars["pauseKeyPressed"]:
                ...
                if gameVars["pauseMenuScreen"] == 0:
                    ...
                    if pauseMenuWeapons[gameVars["pauseMenuRow"]][gameVars["pauseMenuCol"]] != -1:
                        ...
                        screen.blit(playerAssets[currentLevel.player.weapon], (0, 0), (58, 0, 32, 32))
                    ...
                    for i in range(4):
                        for j in range(2):
                            ...
                            if pauseMenuWeapons[i][j] in currentLevel.player.weaponOwned:
                                ...
                                for k in range(1, currentLevel.player.weaponEnergies[pauseMenuWeapons[i][j]] + 1):
                                    x = 16 + 2 * (k - 1)
            ...
            if currentLevel.transitionTimer > 0 or gameVars["menuTransitionTimer"] > 0:
